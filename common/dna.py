...
class PatternDNA(object):
    ...
    def _obtain_t_strand(self, protein):
        ...
        for amino_acid in protein:
            codon_chooser = inherent.codons[1][numpy.where(inherent.codons[0] == amino_acid)]
    ...
    def _obtain_c_strand(self, template_strand):
        ...
        for bases in input_strand:
            strand[0].insert(0, inherent.pairing[1][numpy.where(inherent.pairing[0] == bases[0])][0])
            if bases[1] != " ":
                strand[1].insert(0, inherent.pairing[1][numpy.where(inherent.pairing[0] == bases[1])][0])
            else:
                strand[1].insert(0, " ")
    ...
    def get_dna_fragment(self, start_position=0, stop_position=-1):
        """
        ...
        if start_position >= 0:
            information = [["".join(self._t_strand[0])[start_position: stop_position],
                            "".join(self._t_strand[1])[start_position: stop_position]],
                           ["".join(self._c_strand[0])[start_position: stop_position],
                            "".join(self._c_strand[1])[start_position: stop_position]]]
        else:
            information = [["".join(self._t_strand[0][start_position:]),
                            "".join(self._t_strand[1][start_position:])],
                           ["".join(self._c_strand[0][start_position:]),
                            "".join(self._c_strand[1][start_position:])]]
